			...
			for (int y = 0; y <= 1; y++)
				...
				// edge refers to our current node
				{
					...
					// cycle found
					{
						...
						int[] inv = invert(p);
						PVector c1 = intersection(linesList.get(path[0]),
								linesList.get(path[1]));
						PVector c2 = intersection(linesList.get(path[1]),
								linesList.get(path[2]));
						PVector c3 = intersection(linesList.get(path[2]),
								linesList.get(path[3]));
						PVector c4 = intersection(linesList.get(path[3]),
								linesList.get(path[0]));
						if (isNew(p) && isNew(inv) && isConvex(c1, c2, c3, c4)
								&& nonFlatQuad(c1, c2, c3, c4)
								&& validArea(c1, c2, c3, c4, 10000, 300000)) {
							cycles.add(p);
						}
					}
				}
